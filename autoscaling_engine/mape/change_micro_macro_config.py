...
def add_config_sections(f_name, add_list, ignore_list, serv_option):
    ...
    for curr in add_list:
        ...
        if not (any(substring in curr for substring in ignore_list)):
            ...
            config.set(curr, 'mem_up_lim', '0.6')
            ...
            config.set(curr, 'auto_scale', 'True')
...
def change_config(micro_f_name, macro_f_name, ignore_micro_list, ignore_macro_list):
    ...
    #micro_f_name = "/home/ubuntu/Elascale_secure/config/microservices.ini"
    #macro_f_name = "/home/ubuntu/Elascale_secure/config/macroservices.ini"
    #ignore_micro_list = ["EK_monitor", "beats", "elascale"]
    #ignore_macro_list = []

    #monitor_node = util.run_command("/home/ubuntu/Elascale_secure/./get_node_role.sh monitor").output
    #ignore_macro_list.append(monitor_node.split(":")[0])
    ...
    microservices = util.run_command("sudo docker service ls --format '{{.Name}}'").output
    curr_services = get_names(microservices, "micro")

    add_config_sections(micro_f_name, curr_services, ignore_micro_list, "micro")

    macroservices = util.run_command("sudo docker node ls --format '{{.Hostname}}'").output
...
if __name__=="__main__":
    ...
    os.environ["IGNORE_MICRO_LIST"] = "EK_monitor,beats,elascale"
    os.environ["IGNORE_MACRO_LIST"] = ""

    change_config("/home/ubuntu/Elascale_secure/config/microservices.ini","/home/ubuntu/Elascale_secure/config/macroservices.ini", os.environ["IGNORE_MICRO_LIST"].split(","), os.environ["IGNORE_MACRO_LIST"])
"""
